...
export function useSettings() {
  ...
  const [pendingAvatarUploadUri, setPendingAvatarUploadUri] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    setWeeklyReportEnabled(user?.weeklyReportEnabled === true);
    setMonthlyReportEnabled(user?.monthlyReportEnabled === true);
    setName(user?.name ?? '');
  }, [
    user?.budgetAmount,
    user?.currency,
  ...
  const normalizeUpdatedUser = (data: any): User => {
    ...
    const resolvedAvatarUri = keepLocalAvatarPreview
      ? currentLocalAvatarUri
      : avatarFromApi !== undefined
      ? avatarFromApi
      : normalizeImageUri(
          user?.avatarUri ?? user?.avatarUrl ?? resolvedAvatarUrl,
        );
  ...
  const isUnknownAvatarDeleteFieldError = (error: any) => {
    ...
    const message =
      extractApiMessage(error?.response?.data)?.toLowerCase() || '';
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: {
      ...
      if (data.weeklyReportEnabled !== undefined) {
        payload.weeklyReportEnabled = data.weeklyReportEnabled;
      }

      if (data.monthlyReportEnabled !== undefined) {
        payload.monthlyReportEnabled = data.monthlyReportEnabled;
      }
      ...
      if (!useAuthStore.getState().isAuthenticated) {
        ...
        return normalizeUpdatedUser({
          ...
          budgetPeriodStart:
            nextPeriod === 'period'
              ? data.budgetPeriodStart ?? user.budgetPeriodStart
              : null,
          budgetPeriodEnd:
            nextPeriod === 'period'
              ? data.budgetPeriodEnd ?? user.budgetPeriodEnd
              : null,
          currency: data.currency ?? user.currency,
          weeklyReportEnabled:
            data.weeklyReportEnabled ?? user.weeklyReportEnabled,
          monthlyReportEnabled:
            data.monthlyReportEnabled ?? user.monthlyReportEnabled,
          avatarUri: nextAvatarUri,
      ...
      if (wantsAvatarRemoval) {
        const avatarRemovalPayloads: Array<
    ...
    onSuccess: data => {
      ...
      setCurrency(
        normalizeCurrency(data?.currency ?? user?.currency, DEFAULT_CURRENCY),
      );
      ...
      setBudgetPeriodStart(
        nextPeriod === 'period' && typeof data?.budgetPeriodStart === 'string'
      ...
      setBudgetPeriodEnd(
        nextPeriod === 'period' && typeof data?.budgetPeriodEnd === 'string'
      ...
      setMonthlyReportEnabled(data?.monthlyReportEnabled === true);
    },
    onError: (err: any) => {
      ...
      const technical =
        typeof err?.message === 'string' && err.message.trim().length
          ? ` (${err.message.trim()})`
          : '';
      alert(t('common.error'), apiMessage || `${fallback}${technical}`);
    },
  });
  ...
  const reportPreferencesMutation = useMutation({
    mutationFn: (data: {
      weeklyReportEnabled?: boolean;
      monthlyReportEnabled?: boolean;
    ...
    onSuccess: updatedUser => {
      setUser(updatedUser);
      setWeeklyReportEnabled(updatedUser.weeklyReportEnabled === true);
      setMonthlyReportEnabled(updatedUser.monthlyReportEnabled === true);
      queryClient.invalidateQueries({ queryKey: ['users', 'me'] });
    },
    onError: (err: any) => {
      alert(
        ...
        extractApiMessage(err?.response?.data) ||
          t('settings.autoReportsFailed'),
      );
    },
  });

  const onToggleWeeklyAutomaticReport = (enabled: boolean) => {
    reportPreferencesMutation.mutate({ weeklyReportEnabled: enabled });
  };

  const onToggleMonthlyAutomaticReport = (enabled: boolean) => {
    reportPreferencesMutation.mutate({ monthlyReportEnabled: enabled });
  };

  const onSelectLanguage = () => {
    alert(
      t('settings.languageDialogTitle'),
      t('settings.languageDialogMessage'),
      [
        {
  ...
  return {
    ...
    isSavingReportPreferences: reportPreferencesMutation.isPending,
    weeklyReportEnabled,
    monthlyReportEnabled,
    themeMode,
    setThemeMode,
    ...
    onToggleMonthlyAutomaticReport,
    onSelectLanguage,
    onSelectTheme,
  };
}
